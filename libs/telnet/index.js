...
module.exports = class Telnet extends events.EventEmitter {
  ...
  connect(opts) {
    return new Promise((resolve, reject) => {
      ...
      this.loginPrompt = (typeof opts.loginPrompt !== 'undefined' ? opts.loginPrompt : /Username[: ]*$/i)
  ...
  _parseData(chunk, callback) {
    ...
    if (this.state === 'getprompt') {
	  ...
	  else if (utils.search(stringData.trim(), />$/) !== -1) {
        this.state = 'login'
        this._login('enable')
      }
      ...
      if (promptIndex === -1 && stringData.length !== 0) {
        if (utils.search(stringData.trim(), this.pageSeparator) !== -1) {
  ...
  _login(handle) {
    if ((handle === 'username' || handle === 'password' || handle === 'enable') && this.socket.writable) {
